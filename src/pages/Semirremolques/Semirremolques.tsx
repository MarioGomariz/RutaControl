...
export default function Semirremolques() {
  const { semirremolques, isLoading, error, fetchSemirremolques } =
    useSemirremolquesStore();
  const { user } = useAuth();
  const [searchTerm, setSearchTerm] = useState("");
  const [filtroVencimiento, setFiltroVencimiento] =
    useState<FiltroVencimiento>("todos");
  ...
  const filteredSemirremolques = semirremolques.filter((semirremolque) => {
    ...
    const dominioNormalized =
      semirremolque.dominio?.toLowerCase().replace(/\s+/g, "") || "";
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-4 flex flex-col text-gray-800">
      ...
      <p className="text-gray-600 mb-6 pl-9">
        Gestión de flota de semirremolques
      </p>
          ...
          {filteredSemirremolques.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-64 w-full">
              ...
              <p className="text-gray-500 text-xl font-medium">
                {searchTerm
                  ? "No se encontraron semirremolques que coincidan con la búsqueda"
...
function SemirremolqueCard({
  ...
  const hasExpiredDoc = requiredDocFields.some((field) => {
    const status = getExpirationStatus(
      semirremolque[field as keyof Semirremolque] as string,
    );
  ...
  const isEnViaje =
    semirremolque.estado === "en viaje" || semirremolque.estado === "en_viaje";
  const isEnReparacion =
    semirremolque.estado === "en reparacion" ||
    semirremolque.estado === "en_reparacion";
  const isFueraDeServicio =
    semirremolque.estado === "fuera de servicio" ||
    semirremolque.estado === "fuera_de_servicio";
  ...
  return (
    <Link to={`/semirremolque/${semirremolque.id}`} className="block">
      <div
        className={`bg-white rounded-lg shadow-md overflow-hidden border-l-4 hover:shadow-lg transition-shadow ${getBorderColor()}`}
      >
        <div className="p-5">
          ...
          <div className="mb-3">
            <h3 className="font-bold text-lg text-gray-800 truncate">
              {semirremolque.nombre
                ? formatNombrePropio(semirremolque.nombre)
                : "Sin nombre"}
            </h3>
          </div>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex items-center text-gray-600">
              ...
              <span>
                {semirremolque.dominio
                  ? formatMatricula(semirremolque.dominio)
                  : "Sin dominio"}
              </span>
            </div>
            ...
            <div className="mt-4 pt-3 border-t border-gray-100">
              ...
              <div className="space-y-2">
                {requiredDocFields.map((field) => {
                  const value = semirremolque[
                    field as keyof Semirremolque
                  ] as string;
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`px-2 py-0.5 rounded-full font-medium whitespace-nowrap ${badgeColor}`}
                        >
                          {badgeText}
                        </span>
                      </div>
