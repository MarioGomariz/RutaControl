...
export default function Tractores() {
  ...
  const filteredTractores = tractores.filter((tractor) => {
    ...
    const dominioNormalized =
      tractor.dominio?.toLowerCase().replace(/\s+/g, "") || "";
          ...
          {searchTerm ? (
            <>
              ...
              <p className="text-gray-500 text-lg">
                No se encontraron tractores que coincidan con "{searchTerm}"
              </p>
              <button
            ...
            <>
              ...
              <Link
                to="/tractor/new"
                className="mt-3 text-blue-500 hover:text-blue-700 inline-block"
              >
                Agregar un tractor
              </Link>
            </>
...
function TractorCard({ tractor }: { tractor: Tractor }) {
  ...
  const isEnViaje =
    tractor.estado === "en viaje" || tractor.estado === "en_viaje";
  const isEnReparacion =
    tractor.estado === "en reparacion" || tractor.estado === "en_reparacion";
  const isFueraDeServicio =
    tractor.estado === "fuera de servicio" ||
    tractor.estado === "fuera_de_servicio";
        ...
        <div className="p-5">
          ...
          <div className="mb-3">
            <h3 className="font-bold text-lg text-gray-800 truncate">
              {formatNombrePropio(tractor.marca)}{" "}
              {formatNombrePropio(tractor.modelo)}
            </h3>
          </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center text-gray-600">
              ...
              <span>
                {tractor.dominio
                  ? formatMatricula(tractor.dominio)
                  : "Sin dominio"}
              </span>
            </div>
              ...
              <div className="flex items-center">
                <div className="flex-1">
                  ...
                  <p className="font-medium">
                    {formatDate(tractor.vencimiento_rto)}
                  </p>
                </div>
                <div
                  className={`px-3 py-1 rounded-full text-xs font-medium ${isRtoExpired ? "bg-red-100 text-red-800" : isRtoExpiringSoon ? "bg-amber-100 text-amber-800" : "bg-green-100 text-green-800"}`}
                >
                  {isRtoExpired
                    ? "Vencido"
                    : daysUntilRto === 0
