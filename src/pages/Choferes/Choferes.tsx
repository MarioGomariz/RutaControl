...
function ChoferCard({ chofer }: { chofer: Chofer }) {
  ...
  return (
    <Link to={`/chofer/${chofer.id}`} className="block">
      <div
        className={`bg-white rounded-lg shadow-md overflow-hidden border-l-4 hover:shadow-lg transition-shadow ${!chofer.activo ? "border-gray-400" : isExpired ? "border-red-500" : isExpiringSoon ? "border-amber-500" : "border-green-500"}`}
      >
        <div className="p-5">
          {/* Badge de estado */}
          <div className="mb-3 flex flex-wrap items-center gap-2">
            ...
            {(chofer.estado === "en viaje" || chofer.estado === "en_viaje") && (
          ...
          <div className="mb-3">
            <h3 className="font-bold text-lg text-gray-800 truncate">
              {formatNombrePropio(chofer.nombre)}{" "}
              {formatNombrePropio(chofer.apellido)}
            </h3>
          </div>
          ...
          <div className="mt-4 pt-3 border-t border-gray-100">
            <div className="flex items-center">
              ...
              {chofer.fecha_vencimiento_licencia && (
                <div
                  className={`px-3 py-1 rounded-full text-xs font-medium ${isExpired ? "bg-red-100 text-red-800" : isExpiringSoon ? "bg-amber-100 text-amber-800" : "bg-green-100 text-green-800"}`}
                >
                  {isExpired
                    ? "Vencido"
                    : daysUntilExpiration === 0
