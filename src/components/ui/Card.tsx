...
export default function Card({ card }: { card: CardProps }) {
  ...
  return (
    <button
      className="flex flex-col items-center justify-center cursor-pointer bg-white text-black p-8 h-48 rounded-lg shadow-md transition ease-in-out hover:scale-110 hover:text-background-0 relative"
      onClick={() => (window.location.href = card.link)}
    >
